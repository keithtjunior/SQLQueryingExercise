...
select * from analytics where last_updated = (select min(last_updated) from analytics) order by last_updated asc limit 1;
...
select app_name, reviews, min_installs, (sum(min_installs)/sum(reviews)*100) as proportion from analytics where min_installs >= 100000 group by app_name, reviews, min_installs order by proportion desc limit 1;
